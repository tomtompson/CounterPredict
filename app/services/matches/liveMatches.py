...
class HLTVLiveMatches(HLTVBase):
        ...
        try:
            ...
            for idx, match_element in enumerate(match_containers):
                ...
                if match_data and match_data.get('teamA') and match_data.get('teamB'):
                    ...
                    self.logger.info(f"added match {idx+1}: {match_data['teamA']} vs {match_data['teamB']}")
            
            self.logger.info(f"successfully parsed {len(live_matches)} live matches")
            
        except Exception as e:
            self.logger.error(f"error parsing live matches: {e}")
    ...
    def get_live_matches(self) -> dict:
        ...
        try:
            ...
            self.logger.error(f"error in get_live_matches: {e}")
            raise HTTPException(
                status_code=500,
                detail=f"error getting live matches: {str(e)}"
            )
