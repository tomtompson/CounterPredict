...
from app.utils.utils import convert_timestamp_to_user_timezone, extract_date_from_headline
...
class HLTVTodayMatches(HLTVBase):
    ...
    def __parse_match_data(self, match_element, fallback_timestamp: float = None) -> Optional[Dict]:
        """
        ...
        try:
            ...
            self.logger.debug(f"match {match_id}: {team1_name} vs {team2_name} -> timestamp: {match_timestamp}")
    ...
    def __parse_section(self, section_element) -> List[Dict]:
        ...
        """
        matches = []
        match_zones = section_element.xpath(".//div[contains(@class, 'match-zone-wrapper')]")
        ...
        for zone in match_zones:
            ...
            for match_wrapper in match_wrappers:
                match_data = self.__parse_match_data(match_wrapper, fallback_timestamp=section_timestamp)
                if match_data:
                    matches.append(match_data)
                else:
                    self.logger.warning(f"failed to parse match {match_wrapper.get('data-match-id', 'unknown id')}")
        return matches
    ...
    def get_today_matches(self, user_timezone: str = "UTC") -> dict:
        ...
        try:
            ...
            xpath_sections = "//div[contains(@class, 'matches-list-section') and @match-container]"
            ...
            self.logger.info(f"processing {len(sections_to_parse)} day sections (today and tomorrow)")

            all_matches = []
            for idx, section in enumerate(sections_to_parse):
                section_date = extract_date_from_headline(section,self.get_text_by_xpath)
                ...
                self.logger.info(f"section {idx+1} yielded {len(section_matches)} matches")

            for match in all_matches:
                local_info = convert_timestamp_to_user_timezone(match["match_timestamp"], user_timezone, logger=self.logger)
                if local_info:
            ...
            filtered_matches = [m for m in all_matches if m.get("local_date") == today_local_str]

            self.response["matches"] = filtered_matches
            self.response["match_count"] = len(filtered_matches)
            self.logger.info(f"total matches parsed: {len(all_matches)}, filtered for today ({today_local_str} with timezone {user_timezone}): {len(filtered_matches)}")
