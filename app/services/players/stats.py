...
from app.utils.utils import extract_float_from_percentage_number, convert_minutes_to_seconds
...
class HLTVPlayerStats(HLTVBase):
    ...
    def get_player_stats(self) -> dict:
        ...
        """
        # firepower stats
        kills_per_round = float(self.get_text_by_xpath(Players.Stats.KILLS_PER_ROUND))
        kills_per_round_win = float(self.get_text_by_xpath(Players.Stats.KILLS_PER_ROUND_WIN))
        damage_per_round = float(self.get_text_by_xpath(Players.Stats.DAMAGE_PER_ROUND))
        damage_per_round_win = float(self.get_text_by_xpath(Players.Stats.DAMAGE_PER_ROUND_WIN))
        ...
        rating_1_0 = float(self.get_text_by_xpath(Players.Stats.RATING_1_0))
        ...
        pistol_round_rating = float(self.get_text_by_xpath(Players.Stats.PISTOL_ROUND_RATING))

        # entrying stats
        saved_by_teammate_per_round = float(self.get_text_by_xpath(Players.Stats.SAVED_BY_TEAMMATE_PER_ROUND))
        traded_deaths_per_round = float(self.get_text_by_xpath(Players.Stats.TRADED_DEATHS_PER_ROUND))
        ...
        assists_per_round = float(self.get_text_by_xpath(Players.Stats.ASSISTS_PER_ROUND))
        ...
        # trading stats
        saved_teammate_per_round = float(self.get_text_by_xpath(Players.Stats.SAVED_TEAMMATE_PER_ROUND))
        trade_kills_per_round = float(self.get_text_by_xpath(Players.Stats.TRADE_KILLS_PER_ROUND))
        ...
        damage_per_kill = float(self.get_text_by_xpath(Players.Stats.DAMAGE_PER_KILL))

        # opening stats
        opening_kills_per_round = float(self.get_text_by_xpath(Players.Stats.OPENING_KILLS_PER_ROUND))
        opening_deaths_per_round = float(self.get_text_by_xpath(Players.Stats.OPENING_DEATHS_PER_ROUND))
        ...
        opening_success = extract_float_from_percentage_number(self.get_text_by_xpath(Players.Stats.OPENING_SUCCESS_))
        ...
        attacks_per_round = float(self.get_text_by_xpath(Players.Stats.ATTACKS_PER_ROUND))

        # clutching stats
        clutch_points_per_round = float(self.get_text_by_xpath(Players.Stats.CLUTCH_POINTS_PER_ROUND))
        ...
        # sniping stats
        sniper_kills_per_round = float(self.get_text_by_xpath(Players.Stats.SNIPER_KILLS_PER_ROUND))
        ...
        sniper_multi_kill_rounds = float(self.get_text_by_xpath(Players.Stats.SNIPER_MULTI_KILL_ROUNDS))
        sniper_opening_kills_per_round = float(self.get_text_by_xpath(Players.Stats.SNIPER_OPENING_KILLS_PER_ROUND))

        # utility stats
        utility_damage_per_round = float(self.get_text_by_xpath(Players.Stats.UTILITY_DAMAGE_PER_ROUND))
        utility_kills_per_100_rounds = float(self.get_text_by_xpath(Players.Stats.UTILITY_KILLS_PER_100_ROUNDS))
        flashes_thrown_per_round = float(self.get_text_by_xpath(Players.Stats.FLASHES_THROWN_PER_ROUND))
        flash_assists_per_round = float(self.get_text_by_xpath(Players.Stats.FLASH_ASSISTS_PER_ROUND))
        time_opponent_flashed_per_round = float(self.get_text_by_xpath(Players.Stats.TIME_OPPONENT_FLASHED_PER_ROUND))
        ...
        "opening": {
            ...
            "opening_attempt_percentage": opening_attempts_percentage,
