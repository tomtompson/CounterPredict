...
class HLTVTeamAchievements(HLTVBase):
    ...
    def __parse_team_achievements(self) -> list[dict]:
        ...
        try:
            ...
            self.logger.debug(
                f"found {len(placements)} placements, {len(tournament_names)} tournaments",
            )
            ...
            if len(placements) != len(tournament_names) or len(placements) != len(
                tournament_urls,
            ):
                self.logger.warning(
                    f"data length mismatch: placements={len(placements)}, names={len(tournament_names)}, urls={len(tournament_urls)}",
                )
            ...
            for i in range(min_length):
                try:
                    ...
                    name = (
                        trim(tournament_names[i]) if i < len(tournament_names) else None
                    )
                    ...
                    self.logger.exception(f"error parsing achievement {i}: {e}")
                    continue

            self.logger.info(
                f"parsed {len(achievements)} achievements for team {self.team_id}",
            )

        except Exception as e:
            self.logger.exception(f"error parsing team achievements: {e}")
    ...
    def get_team_achievements(self) -> dict:
        ...
        try:
            ...
            self.logger.info(
                f"returning {len(achievements)} achievements for team {self.team_id}",
            )

        except Exception as e:
            self.logger.exception(f"error in get_team_achievements: {e}")
            raise HTTPException(
                status_code=500,
                detail=f"error getting team achievements: {e!s}",
            )
